...
**Claim discovery boundary (in progress, not deployment evidence):** an unbound profile may be presented to an Owner/Admin only as a bounded, adapter-issued, one-time opaque claim handle. The browser never receives a global profile inventory or a profile identifier through discovery. The adapter consumes that handle while registering the exact signed opaque RogerOS binding, rejects expiration/replay/conflicts/malformed claims, and returns only the resulting binding to RogerOS. A failed adapter claim grants no capability; retained RogerOS assignments continue to be visible as their own provenance.
...
The initial Vercel transport sends multipart uploads through a RogerOS Function and therefore enforces a 3.9 MB per-file browser limit plus a 4.0 MB complete-body limit, below Vercel's 4.5 MB hard payload ceiling. The larger validation and adapter maxima are not advertised as deployed browser capacity. Raising the deployed limit requires a separately reviewed private direct-upload flow whose short-lived grant, pending-object lifecycle, server-side validation/normalization, quota reservation, completion proof, and failed-upload cleanup preserve the same project ownership boundary.
...
- M21 Agent Coach is a bounded, GET-only projection of M20's redacted aggregates. Its deterministic review proposals have no apply path, durable record, provider/AI call, approval request, or runtime side effect. They never grant or change permissions, Skills, policies, SOUL, runtime configuration, task state, billing, or production/VPS state; any later change must enter its existing project-scoped governed workflow independently.

## Project search and command
