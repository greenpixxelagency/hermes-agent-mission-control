...
This roadmap summarizes repository evidence. Undefined future work requires explicit planning and approval.

## Completed

- Generic multi-tenant organization, project, membership, authentication, and server-side project context.
- Core project isolation and project-aware RogerOS shell/navigation.
- Team conversations/threads, authoritative Tasks, and Workforce.
- Project Brain with constitution, knowledge, decisions, memory, provenance, and scoped Drive sources.
...
- The owner approved the phased Team Experience direction on 2026-09-14. T1 is complete at `d9d52340ec337b218b3e4680636751cea1edf182`. T2 is split into the Hermes/VPS T2A assignment-bound transcription contract and RogerOS T2B managed assets/UI. The bounded localhost T2 milestone is complete: the isolated 25/25 contract suite and real HTTP 201 voice-note/transcription/reply flows passed for both `aarav` and `default`, with durable READY receipts and one-time redemption evidence. T3 may proceed locally from the committed T2 closeout. Preview durability, migration, TLS binding, deployment, production, and `main` remain separate readiness gates rather than local T3 prerequisites.
- T3 now has a repository-local foundation from the exact T2 closeout: integrated Overview/Model/Profile files/Skills/MCP UI, additive project-owned mutation/file-version/MCP state, a strict signed adapter client, CAS/idempotency/compensation services, and deterministic contract tests. It is not a live-Hermes claim: local migration application and the separate isolated VPS adapter/two-profile acceptance remain gates.
...
- The Hermes control desk is an in-progress, single-project-first experience. Its dynamic roster, governed bot lifecycle, in-RogerOS conversation and same-project mentions, signed opaque-ID profile binding, and approved-model selection are implemented in this checkout. Browser viewing/takeover, observation-based teaching, bounded routine mutation, managed MCP configuration, and gateway restart remain unavailable until the staging adapter supplies assignment-scoped contracts for each capability. Their future implementation must retain the project-scoped authority described in `ROGEROS_ARCHITECTURE.md`; restoring multi-project selection later is a presentation change, not permission to relax that boundary.
