...
export async function reconcileHermesBotAssignment(context: ProjectContext, employeeProjectAssignmentId: string, adapter: HermesRuntimeAdapter = hermesRuntimeAdapter) {
  ...
  try {
    if (drifted) {
      const ensured = await adapter.ensureBot({ profileId: desired.profileId, displayName: desired.displayName, description: desired.description }); validateProfile(ensured.profileId, desired.profileId)
