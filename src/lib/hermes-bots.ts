...
export async function reconcileHermesBotAssignment(context: ProjectContext, employeeProjectAssignmentId: string, adapter: HermesRuntimeAdapter = hermesRuntimeAdapter) {
  ...
  try {
    if (drifted) {
      const ensured = await adapter.ensureBot({ profileId: desired.profileId, projectKey: desired.projectKey, employeeKey: desired.employeeKey }); validateProfile(ensured.profileId, desired.profileId)
