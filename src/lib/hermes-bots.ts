...
function adapterFailure(error: unknown) {
  ...
  if (error instanceof Error && /^HERMES_ADAPTER_\d{3}(?:_[a-zA-Z0-9_]{1,80})?$/.test(error.message)) return new HermesBotError(error.message)
...
export async function reconcileHermesBotAssignment(context: ProjectContext, employeeProjectAssignmentId: string, adapter: HermesRuntimeAdapter = hermesRuntimeAdapter) {
  ...
  try {
    if (drifted) {
      const ensured = await adapter.ensureBot(desired); validateProfile(ensured.profileId, desired.profileId)
