...
export class TeamAvatarError extends Error {
  constructor(
    public code: "FORBIDDEN" | "INVALID_AVATAR" | "ASSIGNMENT_NOT_FOUND",
  ) {
    super(code);
  }
}
...
export async function setTeamAvatarPreset(
  context: ProjectContext,
  assignmentId: string,
  presetKey: unknown,
) {
  if (!managers.has(context.project.role))
    throw new TeamAvatarError("FORBIDDEN");
  if (
    !isTeamAvatarPresetKey(presetKey) ||
    !assignmentId ||
    assignmentId.length > 160
  )
    throw new TeamAvatarError("INVALID_AVATAR");
  const [assignment, actor] = await Promise.all([
    prisma.employeeProjectAssignment.findFirst({
      ...
      select: { id: true, avatarPresetKey: true, avatarAssetId: true },
    }),
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const result = await tx.employeeProjectAssignment.update({
      where: { id: assignment.id },
      data: { avatarPresetKey: presetKey, avatarAssetId: null },
      select: { id: true, avatarPresetKey: true, avatarAssetId: true },
    });
    await recordAuditEvent(
      {
        projectId: context.project.id,
        eventType: "team.avatar.updated",
        actor: { type: AuditActorType.HUMAN, projectMemberId: actor.id },
        targetType: "EmployeeProjectAssignment",
        targetId: assignment.id,
        summary: "Employee avatar preset updated",
        metadata: { previousPresetKey: assignment.avatarPresetKey, presetKey },
      },
      tx,
    );
    return result;
