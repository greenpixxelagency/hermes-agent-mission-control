...
export class TaskAccessError extends Error {
  constructor() { super('TASK_NOT_FOUND'); this.name = 'TaskAccessError' }
}
...
export async function requireTaskAccess(context: ProjectContext, taskId: string) {
  const task = await prisma.task.findFirst({
    ...
    include: {
      assignments: {
        include: {
          ...
          employeeProjectAssignment: {
            include: {
              employee: true,
              runtimeAssignments: { where: { active: true }, include: { runtime: true } },
            },
          },
        },
      },
      ...
      hermesExecutions: { orderBy: { createdAt: 'desc' }, take: 20, include: { runtime: true } },
    },
  })
...
export function taskErrorResponse(error: unknown) {
  if (error instanceof TaskAccessError) return Response.json({ error: 'Not found' }, { status: 404 })
  throw error
}
