...
async function requireCapability(
  ...
  key: "model" | "profileFiles" | "skills" | "mcp",
...
export function validateEmployeeStudioProfileContent(logicalKey: string, content: string) {
...
export async function writeEmployeeStudioProfileFile(
  ...
  const next = validateEmployeeStudioProfileContent(input.logicalKey, content);
...
export async function reconcileEmployeeStudioMcp(
  context: ProjectContext,
  input: { employeeProjectAssignmentId: string; serverKey: string; projectConnectionId: string; enabled: boolean; toolKeys: string[]; idempotencyKey: string },
  adapter: EmployeeStudioAdapter = employeeStudioAdapter,
) {
  assertManager(context);
  if (!mcpKey.test(input.serverKey) || !Array.isArray(input.toolKeys) || input.toolKeys.length > 100 || input.toolKeys.some((key) => !mcpKey.test(key)))
  ...
  const requestMetadata = { serverKey: input.serverKey, projectConnectionId: connection.id, enabled: input.enabled, toolKeys, secretReferenceId: connection.credential.id };
  ...
  if (claimed.replay) return replayResult(claimed.mutation);
  try {
    const receipt = await adapter.reconcileMcp({ binding, mutationId: claimed.mutation.id, serverKey: input.serverKey, enabled: input.enabled, secretReferenceId: connection.credential.id, toolKeys });
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (toolKeys.length) await tx.governedMcpToolAssignment.createMany({ data: toolKeys.map((toolKey) => ({ projectId: context.project.id, mcpAssignmentId: saved.id, toolKey, enabled: input.enabled })) });
