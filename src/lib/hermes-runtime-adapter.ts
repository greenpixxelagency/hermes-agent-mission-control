...
async function request<T>(path: string, init?: RequestInit): Promise<T> { const c = config(); const response = await fetch(`${c.base}${path}`, { ...init, headers: { ...c.headers, ...init?.headers }, cache: 'no-store' }); if (!response.ok) { const payload=await response.json().catch(()=>null) as null|Record<string,unknown>;const raw=[payload?.code,payload?.error,payload?.message].find(value=>typeof value==='string') as string|undefined;const hint=raw?.replace(/[^a-zA-Z0-9]+/g,'_').replace(/^_+|_+$/g,'').slice(0,80);throw new Error(`HERMES_ADAPTER_${response.status}${hint?`_${hint}`:''}`) } return response.json() as Promise<T> }
...
export const hermesRuntimeAdapter: HermesRuntimeAdapter = {
  ...
  ensureBot: spec => request('/bots/ensure', { method: 'POST', body: JSON.stringify(spec) }),
