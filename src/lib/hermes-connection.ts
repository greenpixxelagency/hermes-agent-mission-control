...
export function decryptHermesCredential<T>(envelope: string): T {
  const [version, iv, tag, ciphertext, extra] = envelope.split('.')
  if (version !== 'v1' || !iv || !tag || !ciphertext || extra) throw new HermesConnectionError('INVALID_OAUTH_STATE')
  try { const decipher = createDecipheriv('aes-256-gcm', connectionKey(), Buffer.from(iv, 'base64url')); decipher.setAuthTag(Buffer.from(tag, 'base64url')); return JSON.parse(Buffer.concat([decipher.update(Buffer.from(ciphertext, 'base64url')), decipher.final()]).toString('utf8')) as T } catch { throw new HermesConnectionError('INVALID_OAUTH_STATE') }
}
