...
export function deriveTeamDeskState(input: {
  configured: boolean;
  ...
  return input.retainedCount > 0 || input.inventoryCount > 0
    ? "ACTIVE_ROSTER"
    : "READY_EMPTY";
}

export function teamDeskAllowedActions(
  role: ProjectRole,
  state: TeamDeskState,
  retainedCount: number,
) {
  ...
  return {
    add: managers.has(role) && healthy,
    adopt: managers.has(role) && healthy,
    retry:
      managers.has(role) && retainedCount > 0 && state !== "SETUP_REQUIRED",
    chat: operators.has(role),
  };
}
...
function safeFailure(error: unknown) {
  if (
    ...
    /^HERMES_ADAPTER_(?:NOT_CONFIGURED|\d{3}_[A-Z0-9_]{1,200})$/.test(
      error.message,
    )
  )
...
export async function readTeamDesk(
  ...
  const retained = assignments.flatMap((employee) =>
    employee.runtimeAssignments
      .filter(
        (runtime) => runtime.active && runtime.assignmentState !== "RETIRED",
      )
      .map((runtime) => ({
        ...
        avatarAssetId: employee.avatarAssetId,
  ...
  if (!hermesAdapterConfigured())
    return {
      ...common,
      state: "SETUP_REQUIRED" as const,
      failureCode: "HERMES_ADAPTER_NOT_CONFIGURED",
      inventory: [],
      claimableProfiles: [],
      allowedActions: teamDeskAllowedActions(
        context.project.role,
        "SETUP_REQUIRED",
        retained.length,
      ),
      provenance: "RETAINED_ROGEROS_ASSIGNMENTS" as const,
    };
  try {
    ...
    let claimableProfiles = [] as Awaited<
      ReturnType<HermesRuntimeAdapter["listClaimableProfiles"]>
    >;
    ...
    const state = deriveTeamDeskState({
      configured: true,
      adapterReachable: true,
      retainedCount: retained.length,
      inventoryCount: inventory.length,
      needsAttention: retained.some(
        (row) =>
          row.provisioningState === "FAILED" ||
          row.reconciliationState === "FAILED",
      ),
    });
    const allowedActions = teamDeskAllowedActions(
      context.project.role,
      state,
      retained.length,
    );
    return {
      ...
      allowedActions: {
        ...allowedActions,
        adopt: allowedActions.adopt && !claimFailureCode,
      },
      provenance: "SIGNED_PROJECT_BINDING" as const,
    };
  } catch (error) {
    return {
      ...common,
      state: "ADAPTER_UNAVAILABLE" as const,
