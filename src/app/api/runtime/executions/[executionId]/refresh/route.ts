...
import { refreshHermesExecution } from '@/lib/hermes-runtime'
...
export async function POST(request: Request, { params }: { params: Promise<{ executionId: string }> }) {
  try {
    ...
    return NextResponse.json({ execution: safeExecution(await refreshHermesExecution(context, executionId)) })
  } catch (error) {
