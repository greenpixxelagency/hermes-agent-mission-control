...
export default async function ModulePage({ params }: { params: Promise<{ projectSlug: string; module: string }> }) {
  try {
    const values = await params; const context = await requireProjectContextBySlug(values.projectSlug); const item = modules[values.module as keyof typeof modules]; if (!item) notFound(); if(values.module==='market') return <AppMarketWorkspace project={context.project}/>; const Icon = item.icon
    return <div className="hq-rise"><PageHeader eyebrow={`${context.project.name} · ${item.eyebrow}`} title={item.title} description={`${item.description} This workspace is planned and is not available to configure yet.`} action={<StatusPill tone="neutral">Coming soon</StatusPill>} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{item.cards.map(card => <section className="panel" key={card[0]}><EmptyState icon={<Icon />} title={card[0]} description={card[1]} /></section>)}</div></div>
