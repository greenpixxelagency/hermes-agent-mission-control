...
export default async function ModulePage({ params }: { params: Promise<{ projectSlug: string; module: string }> }) {
  try {
    const values = await params; const context = await requireProjectContextBySlug(values.projectSlug); const item = modules[values.module as keyof typeof modules]; if (!item) notFound(); const Icon = item.icon
