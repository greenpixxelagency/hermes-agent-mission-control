...
1. Read `ROGEROS_ARCHITECTURE.md`, `ROGEROS_STATUS.md`, `ROGEROS_ROADMAP.md`, `ROGEROS_DECISIONS.md`, `ROGEROS_RUNBOOK.md`, and `ROGEROS_TEAM_EXPERIENCE_PLAN.md`.
...
15. The owner has authorized RogerOS work to use the signed-in Google Cloud project for Google OAuth configuration when needed. Reuse an existing client where appropriate, keep client secrets out of source control and chat, and verify the live browser flow after configuration. Before creating or rotating a persistent OAuth secret or saving a Cloud-side credential change, obtain the required action-time confirmation.
