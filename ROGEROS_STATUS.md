...
> **T2 localhost integration correction (2026-09-14):** The repository now exposes the isolated adapter's concrete fixed-origin callback `POST /assets/{assetId}/redeem` and exempts only that exact path from interactive session middleware so its short-lived single-use bearer verifier can run. The existing project-authorized browser asset route remains unchanged. Environment names and the two-direction localhost tunnel boundary are documented; no VPS, Vercel, production, database, or secret state changed. Focused asset tests, TypeScript, scoped lint, and the production build pass. Real end-to-end voice remains pending the dedicated VPS workstream's temporary isolated tunnel/configuration handoff and must stay default-deny until that evidence returns.

> **T2 local implementation and isolated-runtime gate (2026-09-14):** The `codex/t2-voice-architecture` checkout now implements the bounded RogerOS managed-asset foundation, uploaded employee avatars, document/image message cards, authorized playback/download, retention cleanup, assignment-bound one-time voice redemption, MediaRecorder review/retry UI, and the default-deny T2A capability projection. The additive migration uses composite project relations for message, asset, actor, conversation, runtime, assignment, and profile identities. The current Vercel server-upload transport intentionally limits each browser file to 3.9 MB so multipart requests remain below Vercel's 4.5 MB Function payload ceiling; the storage/adapter validation maxima remain upper bounds for a later reviewed direct-upload transport. Local Prisma validation/generation and schema synchronization pass, the complete repository suite passes 55/55, scoped lint and TypeScript pass, the production build passes, and authenticated desktop/mobile acceptance observes the existing 6/7 Team roster with voice correctly unavailable while T2A is disabled. Separately, the isolated `/opt/rogeros-hermes-t2a-acceptance` service and `rogeros-hermes-t2a-v021` runtime passed 24/24 offline contract tests, two-profile routing before/after restart, native synthetic-audio transcription, replay/expiry/MIME/digest/length denial, and rollback checks. It remains disabled and bound to `127.0.0.1:9098`; no existing adapter, public route, six-profile gateway, production deployment, or `main` state changed. T2 is not yet a live completion claim: private Preview Blob storage, additive Preview migration, dedicated TLS exposure, fixed Preview redemption origin, exact staging bindings, and end-to-end voice-note response acceptance remain required.

> **T2 voice/attachment architecture gate (2026-09-14):** Read-only VPS verification found the connected adapter attachment contract absent: Bot Chat accepts only `message` and `correlationId`, and assignment capabilities advertise no attachment policy. The same inspection verified Hermes v0.21.0 and all six profile gateways healthy and confirmed Hermes already has reusable voice transcription building blocks. Voice notes are therefore feasible but not implemented in RogerOS. `ROGEROS_T2_VOICE_ATTACHMENT_CONTRACT.md` now defines the required T2A Hermes/VPS contract and T2B RogerOS managed-asset integration. No upload, schema migration, adapter mutation, VPS change, deployment, or production action has occurred. T2B and T3 remain blocked until T2A returns `PROVEN` evidence and a durable private object store is approved.
