...
> **Control-desk staging acceptance (2026-09-08):** `phase-3` Preview deployed `fb98d4d`; the missing nullable `Project.description` field was corrected by the additive staging-only migration `20260908040000_add_project_description`, and all 25 repository migrations are now applied. Chief of Staff reconciliation then reached the protected adapter but failed closed with the sanitized `HERMES_ADAPTER_500_BOT_INTERNAL_ERROR` result. No opaque binding, model enablement, browser/takeover, teaching, MCP, routine, gateway, or VPS state was changed. The next action is isolated staging-adapter/Hermes diagnosis in the dedicated VPS workstream.
