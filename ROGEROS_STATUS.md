...
> **Reliable localhost startup (2026-09-14):** `npm run dev:local` now owns the complete Windows-local startup path: persistent per-user OAuth/database/managed-asset state outside disposable worktrees, exact localhost configuration validation, isolated PostgreSQL startup, local-only schema push, idempotent owner/catalog seed, and webpack Next.js launch on port 3001. A full stopped-state restart passed with the database/schema/seed healthy, authenticated Team HTTP 200, managed voice asset HTTP 206, and a correctly formed Google OAuth redirect. Production, staging, Hermes/VPS, and tracked secrets were untouched.
